...
export default (editor, opt = {}) => {
  ...
  let coreMjmlModel = {
    ...
    toHTML() {
      /*
       * This function used to respect `void` (self closing) components.
       * However, due to https://github.com/artf/grapesjs-mjml/issues/149
       * I'm disabling it for now.
       * The issue is happening because when the Parser imports the elements,
       * it just uses `Element.innerHTML` to convert the string to DOM.
       *
       * It appears that browsers can understand self-closing tags ONLY
       * on elements that are defined as self-closing like <img /> and <br />
       * but since mjml tags like <mj-font> are unknown to the browser, it will
       * not treat them as self-closing. Therefore
       * <mj-font ... />
       * <mj-font ... />
       *
       * is parsed as
       ...
       * </mj-font>
       *
       * This causes a lot of rendering issues of course.
       *
       * Proper solution would be to parse the string using
       * DOMParser API as XML and iterate over each XML Element
       * and create a DOM Element out of it.
       * However, I'm afraid of performance or stability issues.
       *
       * So for the moment I'm disabling self-closing tags on this renderer.
       * That wont resolve the issue. Grapesjs-Mjml will still be unable to
       * parse self-closing tags.
       * This only makes sure that code generated by grapesjs-mjml will be
       * parse-able by grapesjs-mjml for later.
       */
      ...
      code += `<${tag}${strAttr}>` + model.get('content');
